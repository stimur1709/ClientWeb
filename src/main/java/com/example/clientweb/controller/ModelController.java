...
public interface ModelController<E extends Model> {
    ...
    ResponseEntity<?> save(@RequestParam(name = "file", required = false) MultipartFile file,
    ...
    ResponseEntity<Page<E>> getPage(@RequestParam(value = "itemType", defaultValue = "0") int itemType,
                    @RequestParam("page") int page, @RequestParam("size") int size,
                    @RequestParam(value = "reverse", defaultValue = "true") boolean reverse,
                    @RequestParam(value = "sort", defaultValue = "id") String sort);

    @GetMapping("/{id}")
    ResponseEntity<E> getEducation(@PathVariable("id") int id);
}
